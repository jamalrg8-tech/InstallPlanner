...
| File | Purpose |
|---|---|
| `InstallPlanner.html` | The main app — project schedule, Team Allocation, Dashboard, WA Tracker, and Daily Schedule tabs. |
...
- **Daily Schedule — a new tab that replaces the daily WhatsApp message.** Instead of typing up "Mark's Team: ... / Albert's Team: ..." and sending it to the group chat every morning, the plan now lives in its own tab (**Daily Schedule**, next to WA Tracker) that everyone sharing the planner sees live. Date arrows (‹ ›) and a **Today** button move between days — every day's plan is kept, so you can page back through history, not just see today. **+ Add task** creates a blank task at the very top of the page (in its own "New task — pick a team" group until you assign one, so a freshly added task is never buried below the existing cards); pick who it's for from the **Team** dropdown (the same team roster as the rest of the app — choose **+ Add new team…** right there if it's someone new, e.g. Norberto), type the **Project / location** (autocompletes against your current project list, but you can type anything — not every daily task is a formal project), what needs to be done, and an optional **Comments** field for anything extra. Tasks are grouped into a card per team as soon as a team is picked — exactly like the old message's "Team name: bullet list" layout — and whichever team's card you just touched (a new task, a newly assigned team) moves to the top of the page, so you never have to hunt for what you just entered. A project can have as many teams tagged onto it as you like: each task has one primary **Team**, plus a **+ Support teams** button next to it for tagging one or more additional teams helping on that same task (a small checklist popup, same pattern as the schedule's own Helper teams below) — so "Mark's Team, with Albert and Sammy helping" is one card, not three. Delete a task with its ✕ (confirm-and-Undo, same safety net as everywhere else). **View report** opens a clean, read-only, grouped-by-team summary of the selected day — the same shape as the old WhatsApp message, including any support teams and comments — for a quick look or to read off to someone. **Export PDF** saves the selected day's plan, grouped by team (main team, support teams, project, task, comments), as a landscape PDF if you want a printed or emailed copy. Saves and syncs through the same shared/local backend as everything else.
- **WA Tracker tab: search and "Group by project"** — the **WA Tracker** tab inside the main app now has the same search box and **Group by project** toggle the standalone factory link already had: search filters the list to rows whose Project Name contains what you type, and grouping clusters every row sharing a Project Name under a collapsible banded header. Both are shared per-device with the factory link (search itself isn't saved, but whether grouping is on and which groups you've collapsed carry over between the two, the same way column widths already do), so however you like to view a job's line-items looks the same whichever page you're on.
