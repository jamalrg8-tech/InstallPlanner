...
- **"View all projects" report** — a button at the top of the Dashboard tab opens a read-only report listing every project with all thirteen fields (Project Name, Client, Scope, Glass/Frame Delivery Date and Comments, Start Date, Finish Date, % Complete, Assigned Team, Bonding Required, Comments) — handy for a quick print-friendly-looking overview or for scanning everything at once without touching the editable schedule.
...
Use the **Import Excel / CSV** button inside the app itself — it recognizes common header names (Project Name, Client, Scope, Glass/Frame Delivery Date, Glass/Frame Delivery Comments, Start Date, Finish Date, % Complete, Assigned Team, Comments, Bonding Required) in any order. If your sheet uses different headers, the closest columns in `HEADER_MAP` (near the top of the script) can be extended to match.
