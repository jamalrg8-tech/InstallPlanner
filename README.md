...
- **Fixed: date picker closing on its own arrows** — clicking the up/down or month-navigation arrows inside a Start/Finish date field's calendar popup used to immediately close the calendar. Two separate causes were involved: every keystroke was triggering a full page refresh (now delayed until you're done with that field), and — the bigger cause — the browser was auto-closing the popup simply because that field sits inside a "frozen" (sticky-positioned) column while you scroll. While a date field is focused, its column now briefly drops out of that frozen behavior so the calendar's position stays put; it returns to normal the moment you're done.
- **Fixed: dates now always show as dd/mm/yyyy** — a native date field's displayed format is controlled by the browser/OS, not by the app, which is why Safari and Chrome could disagree. Start and Finish dates now always display as dd/mm/yyyy no matter which browser or device opens the planner (the calendar picker and keyboard entry both still work exactly as before).
