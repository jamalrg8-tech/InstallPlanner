...
If `InstallPlanner.html`, `wa-tracker.html`, or `wa-shared.js` changes, bump `CACHE_NAME` in `sw.js` (e.g. `install-planner-v26`) so visitors' browsers pick up the new version instead of an old cached copy.
...
Anyone who opens that link sees **only** the WA production tracker — the same rows, in the same layout, with full ability to add rows, edit any field, delete rows, and import a spreadsheet. There are no tabs, no project schedule, no dashboard, no team allocation, and no way to navigate to any of them from that page — it's a separate, self-contained page, not a restricted view inside the main app.
