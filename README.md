...
A self-contained, installable web app for planning installation work: project name, client, scope, start/finish dates, % complete, assigned team, and comments. Each project's install window is also shown as a highlighted block on a day-by-day calendar that starts July 2026 and automatically grows to cover every project's dates — open it from the Dashboard's **View schedule grid** button.
...
- **"View all projects" report** — a button at the top of the Dashboard tab opens a read-only report listing every project with all eight fields (Project Name, Client, Scope, Start Date, Finish Date, % Complete, Assigned Team, Comments) — handy for a quick print-friendly-looking overview or for scanning everything at once without touching the editable schedule.
...
Use the **Import Excel / CSV** button inside the app itself — it recognizes common header names (Project Name, Client, Scope, Start Date, Finish Date, % Complete, Assigned Team, Comments) in any order. If your sheet uses different headers, the closest columns in `HEADER_MAP` (near the top of the script) can be extended to match.
