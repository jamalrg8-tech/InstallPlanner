...
A self-contained, installable web app for planning installation work: project name, client, scope, material/glass/frame delivery dates, start/finish dates, % complete, assigned team, comments, and whether all materials have been delivered. Each project's install window is also shown as a highlighted block on a day-by-day calendar that starts July 2026 and automatically grows to cover every project's dates — open it from the Dashboard's **View schedule grid** button.
...
- **Material Delivery Date field** — sits between Project Scope and Start Date, using the same calendar picker as the other dates. It must always be on or before the Start date: picking a delivery date after the current Start date (or a Start date before the current delivery date) is rejected with a toast explaining why, and the field keeps its previous value. Clearing the date is always allowed. Imported spreadsheet rows are checked the same way — a delivery date later than Start is dropped on import (noted in that project's Comments) rather than silently accepted.
- **Glass Delivery Date and Frame Delivery Date fields** — sit right after Material Delivery Date, and follow exactly the same on-or-before-Start-date rule (and the same reject-with-toast / clear-is-always-allowed / import-clamps-and-notes-in-Comments behavior).
- **"Materials delivered?" field** — a simple Yes/No dropdown, after Assigned Team, for marking once material, glass and frame have all actually arrived on site. New projects default to "No"; it carries no date validation of its own.
...
- **"View all projects" report** — a button at the top of the Dashboard tab opens a read-only report listing every project with all twelve fields (Project Name, Client, Scope, Material/Glass/Frame Delivery Date, Start Date, Finish Date, % Complete, Assigned Team, Materials Delivered, Comments) — handy for a quick print-friendly-looking overview or for scanning everything at once without touching the editable schedule.
...
Use the **Import Excel / CSV** button inside the app itself — it recognizes common header names (Project Name, Client, Scope, Material/Glass/Frame Delivery Date, Start Date, Finish Date, % Complete, Assigned Team, Materials Delivered, Comments) in any order. If your sheet uses different headers, the closest columns in `HEADER_MAP` (near the top of the script) can be extended to match.
